...
{:else}
  <button transition:fade {id} class="btn {clazz}" class:btn--muted={muted} disabled={muted} on:click>{label}</button>
{/if}
...
<style>
  ...
  button.btn {
    ...
    border: 1px solid var(--btn-color-border, var(--btn-color-accent, var(--color-accent, #009bff)));
    padding: 8px 16px;
